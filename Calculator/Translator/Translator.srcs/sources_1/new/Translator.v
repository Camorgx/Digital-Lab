...
module Translator(
    ...
    parameter[3 : 0] Add_Return = 9, Final_Add = 10;
    always @(*) begin
        case (state)
            Init: next_state = Init_Push;
            Init_Push: next_state = Check_Top;
            Check_Top: next_state = (top != 0) ? Check_Flag : Add_Return;
            ...
            Add_Return: next_state = Final_Add;
            Final_Add: next_state = Terminate;
    ...
    always @(posedge clk) begin
        case (state)
            ...
            Add_Return: ans[length * 8 +: 8] <= "\n";
            Final_Add: length <= length + 1;
            Terminate: ready <= 1;
        endcase
    end
endmodule
